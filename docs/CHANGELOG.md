...
### Changed
- **代码项目化** — `scripts/md2pdf.py` 拆分为 `scripts/md2pdf/` 包（入口85行+7个子模块），AI 修改时只需关注最小相关文件
...
### Fixed
- **宽表缩放改用 `zoom`** — `transform: scale()` + wrapper 导致跨页时内容乱序、与后续标题重叠。`zoom` 同时缩放视觉和布局，页面断裂后顺序正确
- **2% 容差** — 避免 `scrollWidth` 微小偏差导致所有表格被误缩放
- **宽表居中** — `zoom` 缩小后 `margin: 0 auto` 自然居中
